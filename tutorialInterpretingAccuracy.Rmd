---
title: 'Interpreting accuracy revisited: A refined approach to propositional analysis. R script.'
...
# Introduction
This script refers to the following publication Gieshoff, A.C., Albl-Mikasa, M. (2022). "Interpreting accuracy: A refined approach to interpreting performance analysis" Perspectives [online-first publication]. https://doi.org/10.1080/0907676X.2022.2088296. Please refer to this publication whenever you use this script, as well as for any further information. 
...
The format of the script is R Markdown (.Rmd).
...
#create a list with all relevant files. Ratings tables are excel-tables, so files should end with ".csv". You may change this if for instance you worked with xlsx-files. 
...
```
This data set includes now all variables that are necessary to proceed with the analysis. We can save the working space so that we can more easily load the data later on. 
```{r save image 1, warning=FALSE}
# define the path
setwd(outputPath)
#save the tables created in R (easier to load afterwards)
save.image("1_accuracyDataset1.RData")
```

# Analyses
In this section, we calculate the inter-rater and intra-rater reliability and generate global indicators of performance. The section is divided into 4 sub-sections:
...
```
The boxplot suggests that in dataset 1 professionals rendered more items correctly than students. We can also test this difference more formally by comparing both groups with a Wilcoxon signed rank test:
...
In order to compare the score obtained in each passage, we need first to aggregate the data for each participant and each passage. As the maximum score you can reach in each passage is different, we will again calculate the score as percentage.
...
#weighing is max score, so we can divide score by weighing to obtain the percentage of points obtained
...
# The model gam1 computes a non-linear pattern over time with separate intercepts for each group, i.e., this are two parallel non-linear patterns. 
