...
# Introduction
This script refers to the following publication [publication]. Please refer to this publication whenever you use this script, as well as for any further information.
For reasons of data protection, this tutorial does not include actual data. The intention is to provide the necessary tools to reproduce similar plots and analyses on different data sets. However, the script still follows the structure of the aforementioned publication to enhance clarity.

## A short description of the rating tables
Interpreting accuracy is a method to asses the consistency and completeness of an interpretation by comparing each item (something like a meaning unit) of the interpretation to the corresponding item of the source text.
...
4) "rating": 0 or 1, dependent variable
5) "category": category of the item
6) "weighing": weighing of the category
7) "score": score as computed in the excel-file
8) "episode": thematic section of a source text

## Necessary packages and several definitions
...
```{r paths, echo=F, message=FALSE, warning=FALSE, include=F, results='hide'}
# specify the paths where you saved your rating tables and where you would like to store plots, and RData
# Hidden for data for data protection
inputPath= "U:/Publikationen/interpreting accuracy/data anonymous"
outputPath ="U:/Publikationen/interpreting accuracy/output"
plotPath = "U:/Publikationen/interpreting accuracy/plot"
```
...
Tip: Make sure you close all tables before running this chunk. Otherwise it will throw an error.
...
for(file in filelist){
  ...
  colnames(data)<-c("identifier", "sentence", "unit", "rating","rerating", "category", "weighing", "score", "episode")
...
## Create variables
We retrieve the information to construct the variables from the file name. In this data set, the file name was constructed as follows: participant_expertise.xlsx. We will extract the first part as participant variable and the second one for the a variable expertise.
...
Let's briefly check the data set: Are there any values in rating not available? Are all participants included? Was any participant not imported correctly or not assigned to the correct variables?
...
```{r formatize variables, echo=F, message=FALSE, include=F, results='hide'}
# lapply is used to perform the same operation on different columns. 

# participant, expertise, identifier and episode should be categorical variables and factors
cols<-c("participant","expertise","identifier", "episode")
# turn into factor
...
Here, we calculate the intra-rater reliability between the first and the second rating. 
```{r interrater reliability}
require("irr")
# intuitive approach (https://stats.stackexchange.com/questions/213743/test-retest-of-highly-skewed-dichotomous-variables)
# remove rows with na
dataset1 = dataset1[!is.na(dataset1$rating),]
...
dataset1$agreement = ifelse(dataset1$rating==dataset1$rerating, 1,0)
# calculate percentage
sum(dataset1$agreement, na.rm=T)/nrow(dataset1)

# ICC is not based on simple correlation, but on variance. see also: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4913118/
...
## Total score
We can calculate a total score per participant by adding up all scores per participant.We then plot the scores of both groups in a boxplot.
...
```
We will also add a numeric identifier that will help to produce cleaner plots and can serve as "timeline". 
...
This is not surprising: the beginning (and the end) are rather formulaic. It is plausible to assume that we will find differences more in the middle part. This is were the variable "episodes" comes in.

### Comparison of episodes
In order to compare the score obtained in each episode, we need first to aggregate the data for each participant and each episode. As the score you can reach in each episode is very different, we will again calculate the percentage.
```{r episodes, warning=FALSE}
episodes = data.frame(aggregate(score~participant+episode+expertise, dataset1, base::sum))
maxScore = c(aggregate(weighing~participant+episode+expertise, dataset1, base::sum)[4])[[1]]
episodes = cbind(episodes, maxScore=maxScore)

#weighing is max score, so we can divide score by weighing to obtain teh percentage of points obtained
episodes$percentage = episodes$score/episodes$maxScore

#plot
ggplot(episodes, aes(x=expertise, y=percentage, fill=expertise))+
  facet_grid(~episode)+
  geom_boxplot()+
  scale_fill_manual(values = c(color1, color2),  labels=c(var1, var2), breaks=brks)+
  theme_bw()+
  labs(x="score", y="expertise")+
  ggtitle("interpreting accuracy during different episodes\n7 dataset1")+
  theme(plot.title = element_text(size=16), legend.title=element_text(size=14), legend.text=element_text(size=14),
          axis.title.x=element_blank(), axis.title.y = element_text(size=12),
...
setwd(plotPath)
title="dataset1_episodes"
filename<-paste(format(Sys.time(), "%Y_%b_%d"),title, ".png")
ggsave(filename, dpi=300, width=12, height=8, units="in")

```
According to the boxplot, score of professionals and students differ in each episode.
Now we can conduct a repeated measures anova to see whether this difference is significant.

```{r episodes test}
# see also: https://www.datanovia.com/en/lessons/repeated-measures-anova-in-r/
# test assumptions: outliers
episodes %>% group_by(episode) %>% identify_outliers(percentage)
...
# normality
episodes %>% group_by(episode, expertise) %>% shapiro_test(percentage)
# p > .05 suggests that the normality assumption is not violated

m0 = anova_test(data = episodes, dv = percentage, wid = participant, 
                within = episode, between=expertise)
get_anova_table(m0)
...
ggboxplot(
  episodes, x="expertise", y="percentage",
  fill="expertise", 
  facet.by="episode", ylim=c(0,1))+
  stat_pvalue_manual(comparison, label="p.adj.signif", y.positio= 1)+
  scale_fill_manual(values=c(color1, color2),  labels=c(var1, var2), breaks=brks)+
  scale_y_continuous(expand = expansion(mult = c(0, 0.2)))+
  theme(axis.text.x = element_blank(), axis.ticks = element_blank())
...
setwd(plotPath)
title="dataset1_episodes_anovaEffects"
filename<-paste(format(Sys.time(), "%Y_%b_%d"),title, ".png")
ggsave(filename, dpi=150, width=8, height=8, units="in")
...
# obviously, our pattern is really curvy.. (several steps are left out to save computation time)
